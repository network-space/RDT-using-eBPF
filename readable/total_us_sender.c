...
#define cf                                        \
	for (unsigned char pti = 0; pti < pcif; pti++)  \
		for (unsigned char pdi = 0; pdi < pds; pdi++) \
			f[pti][pdi] = RANDOM_BYTE, fa[pti] = 0;
	cf;
	printf("file info:	");
	for (unsigned char pti = 0; pti < pcif; pti++)
		for (unsigned char pdi = 0; pdi < pds; pdi++)
	...
	c:
		...
		for (unsigned char pti = 0; pti < num_bytes; pti++)
			printf(" %hhu", reb[pti]);
		printf("\n");

		if (num_bytes)
			for (unsigned char aci = 0; aci < reb[0]; aci++)
				fa[reb[0 + 1 + aci]] = 1; //, printf("received, %hhu\n", *reb);
...
gcc total_us_sender.c -o total_us_sender.exe
